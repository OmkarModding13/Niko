...
import { incrementAutoBanCounter, getAutoBanEmbed, saveAutoBanEmbed } from '../utils/database.js';
    ...
    try {
        ...
        if (savedEmbed?.messageId) {
            try {
                ...
                // Old embed not found; create a new one.
            }
        }
...
export async function refreshAutoBanEmbed(client, guild) {
    try {
        const channel = guild.channels.cache.get(AUTO_BAN_CHANNEL_ID);
        if (!channel?.isTextBased?.()) return;

        const savedEmbed = await getAutoBanEmbed(client, guild.id);
        if (!savedEmbed?.messageId) return;

        const existingMessage = await channel.messages.fetch(savedEmbed.messageId).catch(() => null);
        if (!existingMessage) return;

        const banCount = await client.db.get(`guild:${guild.id}:autoBanCount`) || 0;
        ...
        logger.info(`[Auto-Ban] Refreshed anti-bot embed in ${guild.name}.`);
    } catch (error) {
        logger.error('[Auto-Ban] Failed to refresh anti-bot embed on startup:', error);
    }
}
