...
class TitanBot extends Client {
    ...
    async start() {
        ...
        try {
            ...
            // Do not register the same commands globally AND per-guild.
            // Doing both creates duplicate slash-command entries in Discord.
            // Clear legacy guild-scoped commands; the global registration above
            ...
            for (const guild of this.guilds.cache.values()) {
                try {
                    ...
                    logger.info(`Cleared legacy guild-scoped commands for ${guild.id}`);
                } catch (error) {
                    logger.error(`Failed to clear guild commands for ${guild.id}:`, error);
                }
            }
    ...
    setupCronJobs() {
        ...
        // Daily reward reminder check - runs every minute
        ...
        // Runs every hour.
        // The processor itself checks whether the
        // user's weekly/monthly period has actually expired.

        cron.schedule(
            '5 * * * *',
            runSafeTask(
                'leveling_period_check',
                () =>
                    processAllLevelingPeriods(
                        this
                    )
            )
        );
        ...
        () => {
            ...
            process.on(
                ...
                (reason) => {
                    ...
                    // Log loudly with full context;
                    // the central task handler categorizes it.
