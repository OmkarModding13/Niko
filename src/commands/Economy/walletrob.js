...
function cooldownRemaining(userData, now = Date.now(), userId = null) {
    ...
    const expiry = Number(userData?.walletRobCooldownExpiresAt || 0);
...
🚫 **Wallet Robbery cancelled.** Owner ko lootne se pehle permission lena tha, bhai. 😈`
    ...
    const protectionExpiry = Number(targetData.walletProtectionExpiresAt || 0);
    if (protectionExpiry > now) {
        return {
            type: 'blocked',
            embed: warningEmbed(
                '🛡️ Wallet Robbery Blocked',
        ...
        if (remaining > 0) {
            return {
                ...
                embed: warningEmbed(
                    ...
                    `<@${player.id}> is still on Wallet Robbery cooldown for **${formatDuration(remaining)}**.`
                ),
            };
        }
    }
    ...
    if (Math.random() >= BASE_ROB_SUCCESS_CHANCE) {
        for (const player of players) {
            ...
            if (!isBotOwner(player.id)) {
                data.walletRobCooldownExpiresAt = now + POLICE_COOLDOWN;
            }
        ...
        const embed = buildUserErrorEmbed(
            'unknown',
            'The police caught the entire crew! **Every robber loses 1,000 Souls**. Non-owner robbers also receive a **10-minute Wallet Robbery cooldown**.',
    ...
    for (let index = 0; index < players.length; index += 1) {
        ...
        if (!isBotOwner(player.id)) {
            data.walletRobCooldownExpiresAt = now + ROB_COOLDOWN;
        }
    ...
    const embed = successEmbed(
        '🏦 Wallet Robbery Successful',
        `The crew successfully robbed **${SOULS} ${fmt(totalReward)} Souls** from <@${targetUser.id}>'s wallet!`
    );
    ...
    embed.addFields(
        ...
        {
            name: '🎯 Target Wallet Lost',
            value: `${SOULS} **${fmt(totalReward)} Souls**`,
            inline: true,
        },
        {
            name: '⏱️ Next Wallet Robbery',
...
export default {
    ...
    execute: withErrorHandling(async (interaction, config, client) => {
        ...
        if (targetUser.bot) {
            throw createError('Cannot rob bot', ErrorTypes.VALIDATION, 'Bots cannot be targeted by Wallet Robbery.');
        }
        ...
        if (activeLobbies.has(initiator.id)) {
            throw createError(
                ...
                'You already have an active Wallet Robbery lobby.'
            );
        }
        ...
        const row = new ActionRowBuilder().addComponents(
            new ButtonBuilder()
                .setCustomId(`walletrob_join:${lobbyId}`)
                .setLabel('Join Robbery')
                .setStyle(ButtonStyle.Success),
            new ButtonBuilder()
                .setCustomId(`walletrob_cancel:${lobbyId}`)
                .setLabel('Cancel')
            ...
            collector.on('collect', async component => {
                ...
                if (component.user.bot) {
                    await component.reply({
                        content: '❌ Bots cannot join a Wallet Robbery.',
                        ephemeral: true,
                    }).catch(() => {});
                    return;
                }

                if (component.customId === `walletrob_cancel:${lobbyId}`) {
                    ...
                    await finish({
                        embed: warningEmbed(
                            '🏦 Robbery Cancelled',
                            'The robbery leader cancelled the Wallet Robbery.'
                        ),
                    });
                    return;
                }

                if (component.customId !== `walletrob_join:${lobbyId}`) return;
                ...
                if (joinerCooldown > 0) {
                    await component.reply({
                        content: `❌ You are on Wallet Robbery cooldown for **${formatDuration(joinerCooldown)}**.`,
                        ephemeral: true,
                    }).catch(() => {});
                    return;
                }
                ...
                if (activeLobbies.has(component.user.id)) {
                    await component.reply({
                        content: '❌ You are already leading another Wallet Robbery.',
                        ephemeral: true,
                    }).catch(() => {});
                    return;
                }
                ...
                await component.reply({
                    content: '✅ You joined the Wallet Robbery crew.',
                    ephemeral: true,
                }).catch(() => {});
