...
import { sendNikoNotification } from '../../utils/nikoNotification.js';
...
export default {
    ...
    async execute(interaction, config, client) {
        ...
        try {
            ...
            if (error.code === 50007) {
                const notified = await sendNikoNotification(
                    client,
                    targetUser,
                    'DM Could Not Be Delivered',
                    `A staff member tried to send you a DM, but Discord could not deliver it.\n\n` +
                    `Please check your **Direct Messages** privacy setting for this server.\n` +
                    `If you want to receive Niko notifications, make sure DMs are enabled.`
                );

                const fallbackText = notified
                    ? `I couldn't DM ${targetUser.tag}, so I sent them a notification in <#1550119194811572244>.`
